// Main application entry point - v2 (adapted for renamed stores)
...
function setupWebSocket() {
    wsManager.on('MESSAGE_CREATE', (msg) => {
        const data = msg.d;
        messageStore.insertMessage(data.conversationId, data);

        if (data.conversationId !== currentConversationId) {
            const ch = channelStore.findChannel(data.conversationId);
            if (ch) {
...
function renderConversationList() {
    ...
    conversationList.innerHTML = channels.map(ch => {
        ...
        const lastMsg = messageStore.fetchMessages(ch.id).slice(-1)[0];
...
function renderMessages() {
    ...
    const msgs = messageStore.fetchMessages(currentConversationId);
    const currentUser = currentUserStore.getCurrentUser();
